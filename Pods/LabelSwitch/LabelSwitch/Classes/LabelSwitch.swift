...
public protocol  LabelSwitchDelegate : class {
    func switchChangeToState(_ state: SwitchState, control:LabelSwitch) -> Void
}
    ...
    @objc func switchTaped(sender: Any) {
        UIView.animate(withDuration: 0.3) {
            ...
            case .L:
                ...
                self.delegate?.switchChangeToState(.R, control: self)
                self.curState = .R
            case .R:
                self.updateUIState(self.leftUIState)
                self.delegate?.switchChangeToState(.L, control: self)
                self.curState = .L
            }
        }
    }
